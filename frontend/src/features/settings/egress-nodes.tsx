...
function usesManagedClearance(scope: EgressScope, mode: "manual" | "flaresolverr"): boolean {
  // r1 边界：Console 不进入 Web managed clearance 生命周期，即使全局是 flaresolverr 模式。
                ...
                <TableActionCell>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      {usesManagedClearance(node.scope, clearanceMode) && !node.accountBoundProxy ? <DropdownMenuItem disabled={refreshClearance.isPending} onClick={() => refreshClearance.mutate(node.id)}><RefreshCw />{t("settings.egress.refreshClearance")}</DropdownMenuItem> : null}
...
function ClearanceBadge({ node, clearanceMode }: { node: EgressNodeDTO; clearanceMode: "manual" | "flaresolverr" }) {
  ...
  if (usesManagedClearance(node.scope, clearanceMode)) {
...
function ProbeFamilySummary({ family, probe, row }: { family: "IPv4" | "IPv6"; probe: EgressIPProbeDTO; row: 1 | 2 }) {
  ...
  return (
    <div className="contents">
      ...
      <span className={cn("col-start-2 flex min-w-0 max-w-[10rem] items-center gap-1.5 text-left", rowClass)}>
