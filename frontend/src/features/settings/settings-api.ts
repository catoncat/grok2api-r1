...
export type SettingsConfigDTO = {
  ...
  providerConsole: { baseURL: string; userAgent: string; chatTimeout: string; streamIdleTimeout: string };
...
const settingsConfigValidator = hasShape({
  ...
  providerConsole: hasShape({ baseURL: isString, userAgent: isOptional(isString), chatTimeout: isString, streamIdleTimeout: isOptional(isString) }),
