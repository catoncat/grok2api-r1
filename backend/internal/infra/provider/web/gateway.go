...
func (a *Adapter) openGatewayChat(ctx context.Context, credential account.Credential, previousResponseID string, spec ModelSpec, input normalizedChatInput, enforceStreamIdle bool, excludedNodeID uint64) (*http.Response, *infraegress.Lease, *inferencedomain.WebResponseState, string, error) {
	cfg := a.config()
	token, err := a.cipher.Decrypt(credential.EncryptedAccessToken)
	if err != nil {
		return nil, nil, nil, "", err
	}
	// r1: 允许在 /index 或会话级 403 后排除失败节点，最多换一次不同 Egress。
