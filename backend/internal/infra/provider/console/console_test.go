...
// r1 合同：Console UA 回退——直连/非托管节点使用 Console 配置的 User-Agent；
// 只有托管节点（NodeID != 0 且带 UA）才覆盖。
func TestApplyBrowserHeadersPrefersLeaseUserAgentAndFallsBackToConfigured(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			applyBrowserHeaders(request, "test-sso", test.configured, test.lease)
