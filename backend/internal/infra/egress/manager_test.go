...
func TestClientCreationDoesNotHoldManagerLock(t *testing.T) {
	...
	go func() {
		_, _ = manager.selectNode([]domain.Node{{ID: 1, Health: 1}}, "")
		close(selected)
	}()
	select {
	case <-selected:
...
func TestSelectNodeUsesAtomicInflightCounters(t *testing.T) {
	...
	selected, ok := manager.selectNode(nodes, "")
	if !ok || selected.ID != 2 {
		t.Fatalf("selected node = %#v ok=%v, want 2", selected, ok)
	}
	manager.decrementInflight(1)
	selected, ok = manager.selectNode(nodes, "")
	if !ok || selected.ID != 1 {
		t.Fatalf("selected node after release = %#v ok=%v, want stable node 1", selected, ok)
	}
}
...
func TestAffinitySelectionOnlyRemapsAssignmentsFromRemovedNode(t *testing.T) {
	...
	for accountID := 1; accountID <= 10000; accountID++ {
		...
		before, ok := manager.selectNode(nodes, affinity)
		if !ok {
			t.Fatal("selectNode failed")
		}
		after, ok := manager.selectNode(remaining, affinity)
		if !ok {
			t.Fatal("selectNode failed")
		}
...
func TestFlareSolverrDoesNotManageConsole(t *testing.T) {
	cipher, err := security.NewCipher("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=")
	if err != nil {
		t.Fatal(err)
	}
	credentialCookies, err := cipher.Encrypt("cf_clearance=console-manual")
	if err != nil {
		t.Fatal(err)
	}
	repository := &mutableEgressRepository{node: domain.Node{
		ID: 1, Name: "console", Scope: domain.ScopeConsole, Enabled: true, Health: 1, UserAgent: "console-node-agent",
	}}
	solver := &clearanceSolverStub{}
	manager := NewManager(repository, cipher)
	manager.solver = solver
	...
	lease, err := manager.AcquireCredential(context.Background(), domain.ScopeConsole, accountdomain.Credential{
		ID: 42, Provider: accountdomain.ProviderConsole, EncryptedCloudflareCookie: credentialCookies,
	})
	if err != nil {
		t.Fatal(err)
	}
	defer lease.Release()
	if solver.calls != 0 || lease.CFCookies != "cf_clearance=console-manual" || lease.UserAgent != "console-node-agent" {
		t.Fatalf("solver calls=%d lease=%#v", solver.calls, lease)
	}
	if err := manager.RefreshClearance(context.Background(), 1); err == nil {
		t.Fatal("Console node unexpectedly accepted managed clearance refresh")
	}
	if solver.calls != 0 {
		t.Fatalf("Console refresh invoked solver %d times", solver.calls)
	}
}

func TestLeaseDefersManagedClearanceInvalidationUntilProviderFeedback(t *testing.T) {
	cipher, err := security.NewCipher("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=")
	if err != nil {
		t.Fatal(err)
	}
	...
	repository := &mutableEgressRepository{node: domain.Node{ID: 1, Scope: domain.ScopeWeb, Enabled: true, Health: 1}}
	manager := NewManager(repository, cipher)
	manager.clearances[key] = clearanceState{cookies: "cf_clearance=value"}
	lease := &Lease{
		NodeID: 1, Scope: domain.ScopeWeb, nodeScope: domain.ScopeWeb,
		client: client, clearanceKey: key, clearanceManager: manager,
	}
	request, err := http.NewRequest(http.MethodPost, "https://grok.com/test", strings.NewReader("{}"))
	if err != nil {
		t.Fatal(err)
	}
	...
	if manager.clearances[key].invalid {
		t.Fatal("raw 403 invalidated Clearance before provider classification")
	}
	manager.FeedbackForLease(context.Background(), lease, http.StatusForbidden, nil)
	if !manager.clearances[key].invalid {
		t.Fatal("classified anti-bot feedback did not invalidate Clearance")
	}
}
