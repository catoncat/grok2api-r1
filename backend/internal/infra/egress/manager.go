...
func (l *Lease) doRequest(request *http.Request, invalidateForbidden bool) (*http.Response, error) {
	...
	// Raw 403 must not invalidate clearance here. Statsig code:7 is also a 403 and
	// is not an Egress anti-bot signal; provider-side FeedbackForLease decides.
	return response, err
}
...
func (m *Manager) Acquire(ctx context.Context, scope domain.Scope, affinity string) (*Lease, error) {
	lease, _, err := m.acquire(ctx, scope, affinity, true, "", egressNodeFromContext(ctx), 0)
	return lease, err
}
...
func (m *Manager) acquireCredential(ctx context.Context, scope domain.Scope, credential accountdomain.Credential, excludedNodeID uint64) (*Lease, error) {
	...
	// Affinity stays credential-scoped for selection churn control; proxy identity
	// comes from the context account key so linked providers share Resin sticky.
	lease, _, err := m.acquire(ctx, scope, strconv.FormatUint(credential.ID, 10), true, credential.EncryptedCloudflareCookie, credential.EgressNodeID, excludedNodeID)
	return lease, err
}
...
func (m *Manager) AcquireIfConfigured(ctx context.Context, scope domain.Scope, affinity string) (*Lease, bool, error) {
	return m.acquire(ctx, scope, affinity, false, "", egressNodeFromContext(ctx), 0)
}
...
func (m *Manager) acquire(ctx context.Context, scope domain.Scope, affinity string, allowDirect bool, encryptedCredentialCookies string, boundNodeID uint64, excludedNodeID uint64) (*Lease, bool, error) {
	now := time.Now().UTC()
	managedClearance := isGrokWebScope(scope) && m.clearanceMode() == "flaresolverr"
	configured := false
	var available []domain.Node
	if boundNodeID != 0 {
		if excludedNodeID != 0 && boundNodeID == excludedNodeID {
			return m.acquireUnavailableFallback(ctx, scope, affinity, allowDirect, encryptedCredentialCookies, managedClearance, fmt.Errorf("绑定出口节点 %d 已被排除", boundNodeID))
		}
	...
	for _, candidateScope := range fallbackScopes(scope) {
		...
		// r1: any configured node (including disabled/cooling) means the scope is
		// configured and must not silently fall back to direct.
		...
		for _, node := range nodes {
			...
			// Console may only reuse sticky Web Resin nodes; static Web nodes would
			// mix Console feedback into the shared image-path health.
		...
		if selected, ok := m.selectNode(candidateAvailable, affinity); ok {
			available = []domain.Node{selected}
			break
		}
	}
	if len(available) == 0 {
	...
	return m.leaseForNode(ctx, scope, affinity, encryptedCredentialCookies, managedClearance, available[0])
}
...
func (m *Manager) leaseForNodeWithOptions(ctx context.Context, scope domain.Scope, affinity, encryptedCredentialCookies string, managedClearance bool, selected domain.Node, options clientOptions) (*Lease, bool, error) {
	...
	var once sync.Once
	return &Lease{NodeID: selected.ID, NodeName: selected.Name, Scope: scope, ProxyURL: proxyURL, UserAgent: userAgent, CFCookies: cookies, client: client.client, browser: client.browser, sticky: sticky, proxyPool: proxyPool,
		nodeScope: selected.Scope, freshTunnel: freshTunnel, clearanceKey: clearanceKey, clearanceManager: m, release: func() {
			once.Do(func() {
...
func rendezvousScore(affinity string, nodeID uint64) [32]byte {
	var payload []byte
	payload = append(payload, affinity...)
...
func (m *Manager) FeedbackForScope(ctx context.Context, scope domain.Scope, nodeID uint64, status int, transportErr error) {
	...
	// Serialize health transitions so concurrent 403s can confirm-before-poison.
	m.feedbackMu.Lock()
	...
	closeRequestClients(stale)
	crossed := before >= affinityHealthFloor && value.Health < affinityHealthFloor
...
func (m *Manager) FeedbackForLease(ctx context.Context, lease *Lease, status int, transportErr error) {
	...
	// Clearance is invalidated only after the provider classifies a 403 as anti-bot.
	if status == http.StatusForbidden && lease.clearanceManager != nil && lease.clearanceKey != "" {
		lease.clearanceManager.invalidateClearanceKey(lease.clearanceKey, lease.client)
	}
	if lease.nodeScope != "" && lease.nodeScope != lease.Scope {
		if transportErr != nil || status >= http.StatusInternalServerError || (lease.Scope != domain.ScopeBuild && status == http.StatusForbidden) {
			m.clientMu.Lock()
			stale := m.invalidateClientForScopeLocked(lease.NodeID, lease.Scope)
			m.clientMu.Unlock()
			closeRequestClients(stale)
		}
		return
...
func isGrokWebScope(scope domain.Scope) bool {
	// Console must not enter managed Web clearance lifecycle.
	return scope == domain.ScopeWeb || scope == domain.ScopeWebAsset
}
