...
func (m *Manager) acquireCredential(ctx context.Context, scope domain.Scope, credential accountdomain.Credential, excludedNodeID uint64) (*Lease, error) {
	...
	lease, _, err := m.acquire(ctx, scope, strconv.FormatUint(credential.ID, 10), true, credential.EncryptedCloudflareCookie, excludedNodeID)
	return lease, err
}
...
func (m *Manager) acquire(ctx context.Context, scope domain.Scope, affinity string, allowDirect bool, encryptedCredentialCookies string, excludedNodeID uint64) (*Lease, bool, error) {
	now := time.Now().UTC()
	...
	var once sync.Once
	return &Lease{NodeID: selected.ID, NodeName: selected.Name, Scope: scope, ProxyURL: proxyURL, UserAgent: userAgent, CFCookies: cookies, client: client.client, browser: client.browser, sticky: sticky, proxyPool: proxyPool, nodeScope: selected.Scope, clearanceKey: clearanceKey, clearanceManager: m, release: func() {
		once.Do(func() {
			m.mu.Lock()
			m.inflight[selected.ID]--
...
func (m *Manager) FeedbackForLease(ctx context.Context, lease *Lease, status int, transportErr error) {
	...
	// Clearance is invalidated only after the provider classifies a 403 as an
	// anti-bot signal. A raw 403 may instead be Statsig code:7 and must not
	// mutate Egress state before its response body is inspected.
...
func (m *Manager) FeedbackForScope(ctx context.Context, scope domain.Scope, nodeID uint64, status int, transportErr error) {
	...
	case transportErr != nil:
		...
		m.mu.Unlock()
	default:
		// HTTP status describes the upstream response, not proxy endpoint health.
		return
	}
