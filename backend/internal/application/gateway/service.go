...
func (s *Service) VerifyBuildModel(ctx context.Context, accountID uint64, upstreamModel string) error {
	...
	if response.StatusCode == http.StatusForbidden && failure.ModelPermissionDenied && s.selector != nil {
		...
		s.selector.MarkModelQuotaExhausted(verificationCtx, credential, nil, upstreamModel, retryAfter)
	}
	return failure
}
...
attemptLoop:
	...
	handleResponse:
		...
		responseRetryable := isRetryableResponse(response, route.Provider)
		...
		if isTerminalRequestForbidden(credential.Provider, lastFailure) {
			...
			if response.StatusCode == http.StatusTooManyRequests && response.RateLimit != nil && response.RateLimit.Model == route.UpstreamModel {
				...
				// r1: 429 学到的 Team 归属持久化为账号归因；只读更新，失败只记日志不阻断请求。
				if response.RateLimit.TeamID != "" && credential.TeamID != response.RateLimit.TeamID {
		...
		afterTeamRateLimit:
			...
			if credential.Provider == accountdomain.ProviderBuild && lastFailure.ModelPermissionDenied && !s.markBuildChatDeniedAsReauth.Load() {
				// r1: 明确的模型权限拒绝只冷却账号×该模型 24h，不整号 reauth。
				s.selector.MarkModelPermissionDenied(ctx, credential, route.UpstreamModel)
				s.selector.MarkQuotaStateChanged(credential.Provider)
				failureHandled = true
			...
			} else if s.providers.SupportsCredentialRefresh(credential.Provider) && lastFailure.PermanentAccountDenial && !failureHandled {
				if credential.Provider == accountdomain.ProviderBuild {
					if lastFailure.ModelPermissionDenied && !s.markBuildChatDeniedAsReauth.Load() {
						// r1: 明确的模型权限拒绝只冷却账号×该模型 24h，不整号 reauth。
						s.selector.MarkModelPermissionDenied(ctx, credential, route.UpstreamModel)
						s.selector.MarkQuotaStateChanged(credential.Provider)
						failureHandled = true
...
// isBarePermissionDenied 仅匹配 permission-denied 裸码：没有 access-denied / blocked /
// safety 措辞的 403 按请求级未知处理，不冷却、不 reauth、不标记模型拒绝。
