...
func TestNewQuotaViewLeavesUnlabeledUsagePeriodUnknown(t *testing.T) {
	quota := newQuotaView(&accountdomain.Billing{
		IsUnifiedBillingUser: true, UsagePeriodType: "USAGE_PERIOD_TYPE_WEEKLY", CreditUsagePercent: 42.5,
	}, 0, nil, "")
	if quota.Type != QuotaTypeUnknown {
		t.Fatalf("quota = %#v", quota)
	}
