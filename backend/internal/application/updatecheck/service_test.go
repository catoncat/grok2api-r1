...
func TestCheckTreatsForkBuildMetadataAsCurrent(t *testing.T) {
	...
	service := NewService("v3.0.4+r1.1", client)
	snapshot := service.Check(context.Background())
	if snapshot.Status != StatusUpToDate || snapshot.UpdateAvailable || snapshot.CurrentVersion != "v3.0.4+r1.1" || snapshot.LatestVersion != "v3.0.4" {
		t.Fatalf("snapshot = %#v", snapshot)
	}
}
