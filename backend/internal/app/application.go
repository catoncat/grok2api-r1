...
func New(ctx context.Context, cfg config.Config, logger *slog.Logger) (*Application, error) {
	...
	// r1: 启动时 MigrateQuotaMode 已被上游 console_usage_migration 后台任务取代
	// （SyncIncompleteConsoleQuotas 会替换全部合成 console quota 窗口）。
	...
	egressService.UpdateDefaults(infraegress.DefaultUserAgent, cfg.Provider.Console.UserAgent)
	...
	readiness := func(readyCtx context.Context) httpserver.ReadinessSnapshot {
		return readinessSnapshot(readyCtx, startup, runtimeHealth, mediaHealth, modelRepo, accountRepo, providers, auditService)
	}
	...
	return &Application{
		...
		accountRepo: accountRepo, startupAccounts: accountRepo, modelRepo: modelRepo, providers: providers, web: webAdapter, egress: egressManager, egressOps: egressService, startup: startup,
	}, nil
}
...
func consoleProviderConfig(cfg config.Config) consoleprovider.Config {
	return consoleprovider.Config{
		...
		StreamIdleTimeoutSeconds: int(cfg.Provider.Console.StreamIdleTimeout.Value().Seconds()),
	}
}
...
func (a *Application) Run(ctx context.Context) error {
	...
	// r1: 不启用上游 web_quota_startup_catchup（周期全池 Web quota 扫描）。
	// 既定边界：13k+ Web 账号只保留请求后刷新、quota recovery 和管理员手工刷新。
